...
|-------|----------------------|----------------------|--------------------|
| Face detection | Privacy-weighted `0.65R+0.25F1+0.10P`; **adopted primary** `error_hardened_all_raw_rf_iou0_45` (0.9172); RF-DETR reranker recall-first | Decision framework includes hardening scores and **promotes the adopted primary** (not fusion 0.9165); recall floor ≥ 0.85 | Scientific detector claim, formal framework, and App defaults are aligned |
...
| Multimodal detection | Combined presence OAPR **~0.95** | Split composite **0.633348** (screen IoU + text + presence) | **Metric stricter, not system collapse** - report both |
