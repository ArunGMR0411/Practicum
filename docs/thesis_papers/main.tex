...
% Use an em dash only for the Abstract and Index Terms labels.
\makeatletter
\def\abstract{\normalfont
    \if@twocolumn
      \@IEEEabskeysecsize\bfseries\textit{\abstractname}\textemdash\enspace\relax
    \else
...
\def\IEEEkeywords{\normalfont
    \if@twocolumn
      \@IEEEabskeysecsize\bfseries\textit{\IEEEkeywordsname}\textemdash\enspace\relax
    \else
...
failure constraints, with deterministic fallbacks where required. The resulting
framework supports reproducible, compute-aware privacy protection for egocentric
image sharing across varied computing environments, including CPU-only systems.
\end{abstract}
...
\section{RQ1: Face Detection and Scene Profiling}
\label{sec:rq1}
...
\section{RQ2: Face Anonymisation}
\label{sec:rq2}
...
\par\vspace{1.5pt}
\noindent\textit{NullFace} completed the locked protocol and produced meaningful
identity suppression, but malformed facial geometry and slow execution blocked
default promotion.\par\vspace{1.5pt}
\noindent\textit{Latent diffusion} preserved full-frame structure, but frequent
severe synthesis artefacts restricted it to research evidence.
\par\vspace{1.5pt}
\noindent\textit{Reverse Personalization} improved after targeted retry, yet
persistent failures, appearance drift, weak hard cases, and the slowest runtime
prevented operational use.\par\vspace{1.5pt}
\noindent\textit{RiDDLE} achieved the highest observed advanced-method composite,
but pose and gaze changes and its recorded 80-GB accelerator boundary excluded
it from default routing.\par\vspace{1.5pt}
\noindent\textit{FALCO} gave the strongest mean two-attacker identity suppression
...
The results expose a transfer problem in the target domain. Portrait-based
models sometimes generated unrealistic faces when egocentric images contained
unusual pose, gaze, scale, or occlusion. Re-ID, SSIM, and LPIPS did not reliably
expose these errors. Low Re-ID can reward any identity change, while full-frame
utility metrics are dominated by unchanged scene area. Direct review examined
geometry, gaze, pose, expression, blending, and collateral alteration. It found
warped geometry, visible compositing, unrealistic synthesis, and face insertion
at incorrect detector boxes. Figure~\ref{fig:advanced-failures} shows this metric
blind spot. The review is separate from numerical scoring and is not a blinded
...
No method was universally best on the reviewed protocols. Advanced synthesis
sometimes improved numerical privacy, but visual quality, runtime, coverage,
and reliability changed its eligibility. RiDDLE has the strongest numerical
result among the research comparators. FALCO is a privacy-strong comparator.
Neither is a default route.
Layered protection remains the balanced, visually safe default, solid masking
the privacy-first fallback, and blur the practical baseline. The following
...
\hline
Adaptive policy & .8680 & .6634 & .702 & .8320 \\
...
development evidence for the core detectors. Later hardening campaigns
inspected retained held-out residuals. They are therefore reported as post-hoc
hardening, not as a fresh untouched test estimate. The 250-image surface does not
...
Panel A shows screen recall of 1.0000, while environmental lettering and
textures reduced text precision. Combined-risk presence reached $P=0.8333$,
$R=0.9804$, and $F1=0.9009$. The presence score was 0.9458, but the stricter
localisation score was 0.6333. At IoU 0.50, text achieved $P=0.0679$,
$R=0.7600$, and $F1=0.1246$; screens achieved $P=0.4576$, $R=0.6429$, and
$F1=0.5347$. Screen area coverage was 0.8543. Presence and coverage do not prove
complete semantic removal.

Panel B shows equal privacy and utility after rounding. Adaptive routing scored
0.8320, compared with 0.8319 for the fixed policy. The difference was 0.00018
(95\% bootstrap CI: 0.00010 to 0.00026) and arose only from runtime, so it has
little practical meaning. Residual evidence records two text misses, no screen
miss, six readable-text cases, one insufficiently obscured screen, and 37
images with utility below 0.50. These categories are non-exclusive. The
low-overlap screen case and contextual cues remain explicit limitations.
...
project-designed rules, not universal formulas. The materialised research
comparison contains seven objective identifiers:
...
layered outputs, 81 solid masks, and 133 copy-through outputs. Copy-through was
restricted to manually reviewed no-face frames.
...
objective definitions. Each materialised row is backed by per-image routing and
output records, allowing the objective, selected method, applied action,
fallback state, and resulting image to be traced together.
\end{minipage}
\hfill
\begin{minipage}[t]{0.485\textwidth}
...
metric-only route. A separate safety audit found no remaining RF-DETR safety
candidate in any of the 133 retained copy-through frames. Frozen-route replay
also reproduced the 286/81/133 distribution, linking detector evidence to
applied protection end to end.
...
\section{Application Demonstrator}
\label{sec:demonstrator}

The implemented App turns the research into a practical end-to-end privacy
...
plan or inspect and change any stage. Processing remains local, so source images
and generated artefacts stay within the operator-controlled environment. Runtime
estimates prioritise the same device and policy, then a substantial prior run or
the relevant compute-tier estimate.
...
intent to the final pixel operation. Each run receives a unique identifier and
preserves its input manifest, corrected detections, protected outputs, comparison
previews, metadata, and completion report in a consistent directory structure.
This makes results straightforward to inspect and transfer.
\end{minipage}
\hfill
\begin{minipage}[t]{0.485\textwidth}
\centering
...
available hardware before execution begins. Preflight also separates method
availability from suitability: an installed method can remain unrecommended when
its compute or visual-safety boundary does not fit the current run. A
machine-readable registry supplies profile defaults, runtime tiers, model
locations, and fallback order across App modules.
...
to be added without redesigning the operator journey. The same execution core is
also available through a command-line interface for repeatable batch processing,
while the web workflow supports interactive review and demonstration. CASTLE-free
smoke checks exercise the pipeline without redistributing private frames,
supporting research handover and regression testing.
...
RQ2 established layered protection as the balanced, visually safe default, with
solid masking as the privacy-first option and blur as the practical baseline.
Advanced methods broadened the identity-suppression evidence, while runtime,
coverage, and direct visual review determined their eligibility for practical
routing.

RQ3 demonstrated joint face, text, and screen protection within one RGB-image
policy. Combined-risk presence recall reached 0.9804 on the held-out split, with
zero screen-presence misses after development-selected completion. Separate
localisation and residual audits kept the evaluation transparent.
...
Together, the four RQs demonstrate the value of treating egocentric privacy as
a complete processing chain. Failure-inclusive coverage, direct visual review,
residual-risk checks, and recorded handoffs turn measurements into auditable
decisions and ensure that numerical gains correspond to reliable, visually
acceptable outputs.
...
permits, and traceable protected outputs. Its principal contribution is a
reproducible decision framework that combines evidence, visual review, compute
constraints, and safe fallbacks to select protection for each objective.
The retained evidence pack connects locked manifests, per-image routing records,
source-to-table mappings, environment details, and public regression checks.
This supports independent auditing of the reported pipeline while keeping the
private source imagery outside the repository.
...
Google Gemini supported coding in Google Colab, including suggestions and
debugging, and assisted with report language, LaTeX, formatting, and README
generation. The authors reviewed, corrected, and accepted or rejected its
outputs. Gemini did not replace implementation, manual annotation, experimental
execution, visual review, result interpretation, or final judgement. The
authors checked every reported value and claim against the project evidence.
No other generative AI model was used. The authors accept responsibility for
the work and complied with the applicable regulations on declaring Generative
AI use.
\end{minipage}
\hfill
\begin{minipage}[t]{0.485\textwidth}
\section*{References}
\renewcommand*{\bibfont}{\fontsize{6.5}{8.15}\selectfont}
\setlength{\bibitemsep}{1.0pt}
