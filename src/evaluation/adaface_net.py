...
def load_pretrained_model(architecture='ir_50', checkpoint_path=None):
    if architecture != 'ir_50':
        raise ValueError("Only 'ir_50' is supported in the current implementation")
    
    # Mock pytorch_lightning dynamically to load checkpoint safely
    import sys
    ...
    pl_mock.callbacks.model_checkpoint = MagicMock()
    pl_mock.callbacks.model_checkpoint.ModelCheckpoint = MockModelCheckpoint
    sys.modules['pytorch_lightning'] = pl_mock
    sys.modules['pytorch_lightning.callbacks'] = pl_mock.callbacks
    sys.modules['pytorch_lightning.callbacks.model_checkpoint'] = pl_mock.callbacks.model_checkpoint

    model = IR_50()
    statedict = torch.load(checkpoint_path, map_location='cpu', weights_only=False)['state_dict']
