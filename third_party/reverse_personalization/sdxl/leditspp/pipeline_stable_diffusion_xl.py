...
class StableDiffusionXLPipeline(
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if not output_type == "latent":
            # Make sure VAE decode runs with a consistent dtype. The upstream
            # partial-upcast path can leave decoder modules and latents in mixed
            # fp16/fp32 states on newer CUDA/PyTorch stacks.
            ...
            if needs_upcasting and original_vae_dtype == torch.float16:
    ...
    def invert(self,
        ...
        #
        # The upstream implementation partially upcasts the VAE and then decodes
        # `x0` during inversion. On newer CUDA/PyTorch stacks this can leave the
        # decoder in a mixed state where some modules still expect fp16 while the
        # latent tensor has already been promoted to fp32, triggering
        # "expected scalar type Half but found Float". For inversion we prefer the
        # stable path over the memory-optimised one and force a consistent fp32
        # decode whenever the VAE requests upcasting.
        original_vae_dtype = self.vae.dtype
        if self.vae.config.force_upcast:
            self.vae.to(dtype=torch.float32)
            x0_tmp = x0.to(torch.float32)
            image_rec = self.vae.decode(x0_tmp / self.vae.config.scaling_factor, return_dict=False)[0]
            if original_vae_dtype == torch.float16:
                self.vae.to(dtype=torch.float16)
        else:
