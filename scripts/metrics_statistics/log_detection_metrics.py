...
"""Log full-scale detection metrics to W&B when available, else filesystem JSON."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--results-json", default="outputs/detection_full_results.json")
    parser.add_argument("--run-group", default="detection")
    parser.add_argument("--output-json", default="outputs/detection_metrics_log.json")
...
def main() -> int:
    ...
    detection_payload = load_json(PROJECT_ROOT / args.results_json)
    ...
    logger = get_logger("detection_metrics", log_dir=logs_dir)

    run_id = build_run_id("detection")
    enable_wandb = bool(config.get("evaluation_switches", {}).get("enable_wandb", True))
    wandb_mode = resolve_wandb_mode(enable_wandb)
    metrics = flatten_metrics("detection/full/", detection_payload)
    ...
    try:
        ...
        run.summary["reviewed_images"] = int(
            detection_payload.get("ground_truth", {}).get("reviewed_images", 0)
        )
        run.finish()
        tracking_backend = f"wandb_{wandb_mode}"
        logger.info("Logged detection metrics via %s", tracking_backend)
